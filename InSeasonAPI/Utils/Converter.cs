...
namespace InSeasonAPI.Utils
{
    class Converter
    {
        ...
        public List<GnisCountyDefinition> CountyToGnis(int county)
        {
            return (from x in excelQueryFactory.Worksheet<GnisCountyDefinition>() where x.COUNTY_NUMERIC == county select x).ToList();
        }
